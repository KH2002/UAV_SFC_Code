...
- [ppo_solver.py](ppo_solver.py)：基于 PPO 的请求排序策略，环境内部检查能量、计算资源与通信容量约束，策略/价值网络为全连接层。
...
PPO 模型位于 [ppo_solver.py](ppo_solver.py)，当前动作定义为“选择下一条待服务请求（或跳过结束当前时间槽）”，约束由环境内部进行检查：

- 通信容量约束：VNF 间需求不超过链路容量；
- 能量约束：UAV 需满足飞行 + 服务 + 返回基站能耗；
- 计算资源约束：UAV 剩余 CPU 满足 VNF 工作量；
- 时间槽约束：时间槽结束后 UAV 释放忙碌状态并恢复计算资源。

默认策略网络与评估网络使用全连接层（MLP）。如需运行 PPO，请先安装 PyTorch。

更完整的 PPO 文档请见 [docs/ppo.md](docs/ppo.md)。
