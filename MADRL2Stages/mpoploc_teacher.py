# -*- coding: utf-8 -*-
"""MPopLoc copy for MADRL2Stages warm-start teacher data.

This file intentionally copies the root mpoploc.py implementation and adds
slot-level UAV-location tracing for behavior cloning. The original root
mpoploc.py is left untouched.
"""
...
class MPopLocSolver:
    def __init__(self, uavs, requests, locations, num_time_slots=None):
        ...
        self.slot_uav_details = {} # {time_slot: {uav_id: {location_id, request_ids, deployed_vnf_ids}}}
    ...
    def solve(self):
        ...
        for t in range(1, self.num_time_slots + 1):
            ...
            for req in sorted_requests:
                ...
                if s_flag:
                    # 更新真实UAV对象的状态
                    self._allocate_resources(req, assigned_uav_ids, time_slot=t)
    ...
    def _allocate_resources(self, req, assigned_uav_ids, time_slot=None):
        """根据承诺结果，实际更新UAV的状态，并记录逐 slot teacher 轨迹。"""
        deployed_vnf_by_uav = {}
        for loc_id, uav_id in assigned_uav_ids.items():
            vnf = next(v for v in req.vnfs if v.location_id == loc_id)
            deployed_vnf_by_uav.setdefault(int(uav_id), []).append(int(vnf.id))
        ...
        for loc_id, uav_id in assigned_uav_ids.items():
            ...
            if time_slot is not None:
                slot_details = self.slot_uav_details.setdefault(int(time_slot), {})
                detail = slot_details.setdefault(
                    int(uav_id),
                    {
                ...
                detail["location_id"] = int(loc_id)
                if int(req.id) not in detail["request_ids"]:
                    detail["request_ids"].append(int(req.id))
                for vnf_id in deployed_vnf_by_uav.get(int(uav_id), []):
                    if int(vnf_id) not in detail["deployed_vnf_ids"]:
                        detail["deployed_vnf_ids"].append(int(vnf_id))
