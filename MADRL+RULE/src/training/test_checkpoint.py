...
def _decode_action_detail(env: MAPPOSFCEnv, action: int) -> Dict[str, Any]:
    if getattr(env, "action_mode", "") == "location_rule":
        location_id = int(env.decode_action(int(action)))
        return {
            "action_type": "select_location",
            "request_idx": None,
            "request_id": None,
            "vnf_idx": None,
            "vnf_id": None,
            "vnf_location_id": location_id,
        }
...
def _vnf_ref(req_idx: int, req: Any, vnf_idx: int) -> Dict[str, int]:
    vnf = req.vnfs[vnf_idx]
    return {
        "request_idx": int(req_idx),
...
def _vnfs_at_location(env: MAPPOSFCEnv, location_id: Optional[int]) -> List[Dict[str, int]]:
    ...
    for req_idx in range(env._actionable_request_count()):
        ...
        for vnf_idx in (0, 1):
            if int(req.vnfs[vnf_idx].location_id) == int(location_id):
                refs.append(_vnf_ref(req_idx, req, vnf_idx))
    return refs


def _deployed_vnfs_by_agent(env: MAPPOSFCEnv) -> Dict[int, List[Dict[str, int]]]:
    deployed = {int(agent_id): [] for agent_id in env.agent_ids}
    req_by_id = {int(req.id): (req_idx, req) for req_idx, req in enumerate(env.requests)}

    for req_id, claim in env.slot_claims.items():
        req_pair = req_by_id.get(int(req_id))
        if req_pair is None:
            continue
        req_idx, req = req_pair
        for vnf_idx, agent_id in claim.items():
            deployed.setdefault(int(agent_id), []).append(_vnf_ref(req_idx, req, int(vnf_idx)))
...
def main() -> None:
    ...
    slot_fields = [
        ...
        "vnfs_at_selected_location_count",
        "vnfs_at_selected_location_json",
        "deployed_vnf_count",
        "deployed_vnf_ids_json",
    ...
    for ep_idx in range(len(test_dataset)):
        ...
        slot_records[current_slot] = {
            ...
            "selected_locations": {int(aid): None for aid in env.agent_ids},
            "vnfs_at_selected_location": {int(aid): [] for aid in env.agent_ids},
            "deployed_vnfs": {int(aid): [] for aid in env.agent_ids},
        }
        ...
        with torch.no_grad():
            while not done:
                ...
                if action_detail["vnf_location_id"] is not None:
                    rec["selected_locations"][agent_id] = int(action_detail["vnf_location_id"])
                ...
                if bool(info.get("slot_ended", False)):
                    deployed_by_agent = {
                        int(aid): list(vnfs)
                        for aid, vnfs in dict(info.get("deployed_vnfs_by_agent", {})).items()
                    }
                    if not deployed_by_agent:
                        deployed_by_agent = _deployed_vnfs_by_agent(env)
                    for aid in env.agent_ids:
                        selected_location = rec["selected_locations"].get(int(aid))
                        rec["vnfs_at_selected_location"][int(aid)] = _vnfs_at_location(env, selected_location)
                        rec["deployed_vnfs"][int(aid)] = deployed_by_agent.get(int(aid), [])
        ...
        for slot_id, rec in slot_records.items():
            ...
            for aid in env.agent_ids:
                selected_location = rec["selected_locations"].get(int(aid))
                if selected_location is not None and not rec["vnfs_at_selected_location"].get(int(aid)):
                    rec["vnfs_at_selected_location"][int(aid)] = _vnfs_at_location(env, selected_location)
                ...
                slot_writer.writerow(
                    {
                        ...
                        "selected_location_id": rec["selected_locations"].get(int(uav_id)),
                        "vnfs_at_selected_location_count": int(len(rec["vnfs_at_selected_location"].get(int(uav_id), []))),
                        "vnfs_at_selected_location_json": json.dumps(rec["vnfs_at_selected_location"].get(int(uav_id), []), ensure_ascii=False),
                        "deployed_vnf_count": int(len(rec["deployed_vnfs"].get(int(uav_id), []))),
                        "deployed_vnf_ids_json": json.dumps(rec["deployed_vnfs"].get(int(uav_id), []), ensure_ascii=False),
