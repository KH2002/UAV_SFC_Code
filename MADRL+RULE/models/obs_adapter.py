...
def obs_to_tensors(
    ...
    vnf_location_ids = _to_tensor(obs["vnf_location_ids"], dtype=torch.long, device=device).unsqueeze(0)  # [1,P,2]
...
def collate_obs_batch(
    ...
    vnf_location_ids = torch.stack([
        _to_tensor(o["vnf_location_ids"], dtype=torch.long) for o in obs_batch
    ], dim=0)
    context = torch.stack([
    ...
    if device is not None:
        ...
        vnf_location_ids = vnf_location_ids.to(device)
