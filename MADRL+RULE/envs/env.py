...
- 动作空间为离散地点选择：每个 agent 每个 slot 仅选择一个 location
- 在所有 agent 选完 location 后，按规则自动部署可行 VNF，直到无可行候选
"""
...
class MAPPOEnvConfig:
    ...
    claim_reward: float
    sfc_potential_reward: float
    max_potential_sfc_reward_count: float
    deployed_vnf_reward: float
    complete_sfc_reward: float
    empty_slot_penalty: float
    ...
    def action_dim(self) -> int:
        # 每个动作对应一个可选 location（不包含基站0，不需要 END）
...
def load_env_config(yaml_path: Optional[str] = None) -> MAPPOEnvConfig:
    ...
    cfg = {
        ...
        "env": {
            ...
            "claim_reward": 0.05,
            "sfc_potential_reward": 0.0,
            "max_potential_sfc_reward_count": 3.0,
            "deployed_vnf_reward": 0.05,
            "complete_sfc_reward": 2.0,
            "empty_slot_penalty": 0.0,
    ...
    return MAPPOEnvConfig(
        ...
        sfc_potential_reward=float(cfg["env"].get("sfc_potential_reward", cfg["env"]["claim_reward"])),
        max_potential_sfc_reward_count=float(cfg["env"].get("max_potential_sfc_reward_count", 3.0)),
        deployed_vnf_reward=float(cfg["env"].get("deployed_vnf_reward", cfg["env"]["claim_reward"])),
        complete_sfc_reward=float(cfg["env"]["complete_sfc_reward"]),
        empty_slot_penalty=float(cfg["env"].get("empty_slot_penalty", 0.0)),
        terminal_success_bonus=float(cfg["env"].get("terminal_success_bonus", 20.0)),
        max_steps_per_episode=int(cfg["training"]["max_steps_per_episode"]),
    )


class MAPPOSFCEnv:
    """MAPPO+RULE 环境（多智能体轮转地点决策 + 规则部署）。
    ...
    动作定义：
        - [0, num_locations-1]：选择 location_id = action + 1
        - 本模式不需要 END 动作
    """
    ...
    def __init__(
        ...
        self.end_token = -1
        self.action_mode = "location_rule"
        ...
        self.last_slot_deployed_vnfs_by_agent: Dict[int, List[Dict[str, int]]] = {}
    ...
    def step(
        ...
        agent_id: int,
        action: int,
        deploy_score_table: Optional[np.ndarray] = None,
        ...
        if not location_valid:
            invalid_count += 1
            self.agent_slot_location[agent_id] = None
        else:
            self.agent_slot_location[agent_id] = int(location_id)
        ...
        slot_ended = False
        all_end = False  # 兼容字段
        slot_forced_end = False  # 兼容字段
        no_feasible_left = False

        slot_completed = 0
        slot_deployed_vnf = 0
        deployed_vnfs_by_agent: Dict[int, List[Dict[str, int]]] = {
            int(agent_id): [] for agent_id in self.agent_ids
        }
        if self.turn_in_round >= self.cfg.num_uavs:
            # 所有 UAV 都已在本 slot 选完 location，执行规则部署
            ...
            deployed_vnfs_by_agent = {
                ...
                for agent_id, vnfs in self.last_slot_deployed_vnfs_by_agent.items()
            }
        ...
        if slot_ended and slot_completed <= 0:
            reward -= self.cfg.empty_slot_penalty
        ...
        if done and len(self.requests) > 0:
            ...
            reward += success_rate * self.cfg.terminal_success_bonus
        else:
        ...
        info = {
            ...
            "slot_deployed_vnf": int(slot_deployed_vnf),
            "deployed_vnfs_by_agent": deployed_vnfs_by_agent,
    ...
    def get_obs(self) -> Dict[str, object]:
        ...
        vnf_location_ids = self._build_vnf_location_ids()  # [P,2], 0-based location action id
    ...
    def get_action_mask(self, agent_id: int) -> np.ndarray:
        """单 agent 动作掩码，shape = [num_locations]。"""
        selected_by_others = {
            int(location_id)
            for other_agent_id, location_id in self.agent_slot_location.items()
            if int(other_agent_id) != int(agent_id) and location_id is not None
        }

        mask = np.zeros(self.cfg.action_dim, dtype=np.int32)
        for action in range(self.cfg.action_dim):
            location_id = self.decode_action(action)
            if int(location_id) in selected_by_others:
                continue
            if self._is_location_feasible_for_agent(agent_id, location_id):
                mask[action] = 1
        ...
        if int(mask.sum()) <= 0:
            for action in range(self.cfg.action_dim):
                location_id = self.decode_action(action)
                if int(location_id) not in selected_by_others:
                    mask[action] = 1
        if int(mask.sum()) <= 0:
            mask[:] = 1
        return mask
    ...
    def decode_action(self, action: int) -> int:
        """将离散动作映射到 location_id（1..num_locations）。"""
        return int(action) + 1

    # --------------------------- core logic ---------------------------

    def _try_claim(self, agent_id: int, action: int) -> Tuple[bool, Dict[str, object]]:
        req_idx = int(action) // 2
        vnf_idx = int(action) % 2
    ...
    def _is_location_feasible_for_agent(self, agent_id: int, location_id: int) -> bool:
        if location_id <= 0 or location_id > self.cfg.num_locations:
            return False
        prev_locked = self.agent_slot_location.get(agent_id)
        self.agent_slot_location[agent_id] = int(location_id)
        try:
            for req_idx in range(self._actionable_request_count()):
                ...
                if req.is_serviced:
                    continue
                for vnf_idx in (0, 1):
                    if int(req.vnfs[vnf_idx].location_id) != int(location_id):
                        continue
    ...
    def _can_agent_claim_vnf_at_location(
        self,
        agent_id: int,
        req_idx: int,
        vnf_idx: int,
        ...
        """检查 agent 在给定 location 锁定下是否可认领指定 VNF。"""
        prev_locked = self.agent_slot_location.get(agent_id)
        if prev_locked is not None and int(prev_locked) != int(location_id):
            return False
    ...
    def _count_location_sfc_potential(self, agent_id: int, location_id: int) -> int:
        ...
        for req_idx in range(self._actionable_request_count()):
            ...
            for vnf_idx in (0, 1):
                vnf = req.vnfs[vnf_idx]
                if int(vnf.location_id) != int(location_id):
                    continue
                if not self._is_valid_claim(agent_id, req_idx, vnf_idx):
                    continue

                paired_idx = 1 - vnf_idx
                paired_location = int(req.vnfs[paired_idx].location_id)
                for other_agent_id in self.agent_ids:
                    if int(other_agent_id) == int(agent_id):
                        continue
                    if self._can_agent_claim_vnf_at_location(
                        int(other_agent_id),
                        req_idx,
                        paired_idx,
                        paired_location,
    ...
    def _rule_score(self, uav: UAV, req: Request, vnf) -> float:
        """规则打分（分数越高越优先）。

        设计：
        - energy_ratio = 本次部署能耗 / UAV总电量
        - cpu_ratio = 本次部署CPU消耗 / UAV总CPU
        - comm_ratio = 请求通信消耗 / 通信链路总容量
        - score = log(1-energy_ratio) + log(1-cpu_ratio) + log(1-comm_ratio)

        这样当任一占用率接近 1（如 99%）时，对应项会急剧下降，
        不会因为另外两项很小就被优先执行。
        """
        target = self.locations.get(vnf.location_id, uav.location)
        travel_distance = float(utils.calculate_distance(uav.location, target)) if not uav.is_busy else 0.0
        energy_cost = self._estimate_travel_energy(travel_distance) + self._estimate_service_energy(vnf)
        cpu_cost = float(vnf.workload)

        energy_ratio = float(energy_cost / max(config.UAV_BATTERY_CAPACITY, 1e-6))
        cpu_ratio = float(cpu_cost / max(config.UAV_COMPUTATION_CAPACITY, 1e-6))

        comm_demand = float(sum(req.communication_demands.values())) if req.communication_demands else 0.0
        v1_loc = self.locations.get(req.vnfs[0].location_id, config.BASE_STATION_LOCATION)
        v2_loc = self.locations.get(req.vnfs[1].location_id, config.BASE_STATION_LOCATION)
        link_distance = float(utils.calculate_distance(v1_loc, v2_loc))
        link_capacity = float(utils.calculate_link_capacity(link_distance) * max(config.TIME_SLOT_DURATION, 1e-6))
        comm_ratio = float(comm_demand / max(link_capacity, 1e-6))

        eps = 1e-6
        e_margin = max(1.0 - energy_ratio, eps)
        c_margin = max(1.0 - cpu_ratio, eps)
        m_margin = max(1.0 - comm_ratio, eps)
        return float(np.log(e_margin) + np.log(c_margin) + np.log(m_margin))

    def _deploy_slot_by_rule(self, deploy_score_table: Optional[np.ndarray] = None) -> int:
        """在所有 UAV 选完地点后，按完整 SFC 候选排序并逐个部署。

        deploy_score_table: 可选 policy 分数表，shape=[num_uavs, 2*max_pending]。
        第 req_idx 个 SFC 的 vnf0/vnf1 对应列 req_idx*2 和 req_idx*2+1。
        """
        score_table = None if deploy_score_table is None else np.asarray(deploy_score_table, dtype=np.float32)
        self.last_slot_deployed_vnfs_by_agent = {int(agent_id): [] for agent_id in self.agent_ids}
        deployed_count = 0
        while True:
            ...
            for req_idx in range(self._actionable_request_count()):
                ...
                for agent0 in vnf0_agents:
                    for agent1 in vnf1_agents:
                        if agent0 == agent1:
                            continue
                        if score_table is not None and score_table.shape[0] > max(agent0, agent1):
                            vnf0_col = int(req_idx) * 2
                            vnf1_col = int(req_idx) * 2 + 1
                            if score_table.shape[1] > vnf1_col:
                                score = float(score_table[agent0, vnf0_col] + score_table[agent1, vnf1_col])
                            else:
                                score = -float("inf")
                        else:
                            score = (
                                self._rule_score(self.uavs[agent0], req, req.vnfs[0])
                                + self._rule_score(self.uavs[agent1], req, req.vnfs[1])
                            )
                        if not np.isfinite(score):
                            continue
                        candidates.append((score, int(req_idx), int(agent0), int(agent1)))
            ...
            for _, best_req_idx, best_agent0, best_agent1 in candidates:
                if not (
                    self._is_valid_claim(best_agent0, best_req_idx, 0)
                    and self._is_valid_claim(best_agent1, best_req_idx, 1)
                ):
                    continue

                valid0, _ = self._try_claim(best_agent0, int(best_req_idx) * 2)
                valid1, _ = self._try_claim(best_agent1, int(best_req_idx) * 2 + 1)
                if not (valid0 and valid1):
                    continue

                req = self.requests[int(best_req_idx)]
                self.last_slot_deployed_vnfs_by_agent.setdefault(int(best_agent0), []).append(
                    {
                        "request_idx": int(best_req_idx),
                        "request_id": int(req.id),
                        "vnf_idx": 0,
                        "vnf_id": int(req.vnfs[0].id),
                        "location_id": int(req.vnfs[0].location_id),
                    }
                )
                self.last_slot_deployed_vnfs_by_agent.setdefault(int(best_agent1), []).append(
                    {
                        "request_idx": int(best_req_idx),
                        "request_id": int(req.id),
                        "vnf_idx": 1,
                        "vnf_id": int(req.vnfs[1].id),
                        "location_id": int(req.vnfs[1].location_id),
                    }
                )
                deployed_count += 2
    ...
    def _build_vnf_location_ids(self) -> np.ndarray:
        """每个请求两个 VNF 对应的动作索引；padding 位置为 -1。"""
        location_ids = np.full((self.cfg.max_pending, 2), -1, dtype=np.int64)
        for i in range(self.cfg.max_pending):
            if i >= len(self.requests):
                continue

            req = self.requests[i]
            location_ids[i, 0] = int(req.vnfs[0].location_id) - 1
            location_ids[i, 1] = int(req.vnfs[1].location_id) - 1
