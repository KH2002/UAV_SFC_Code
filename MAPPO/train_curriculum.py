# -*- coding: utf-8 -*-
"""MAPPO 课程学习训练入口脚本（仅在数据集调度上增加课程学习）。"""
...
from MAPPO.training import MAPPOTrainer, load_trainer_config
...
def _build_default_curriculum_sizes(total_episodes: int) -> List[int]:
    total = max(int(total_episodes), 1)
    candidates = [1, max(2, total // 4), max(2, total // 2), total]
    sizes: List[int] = []
    for x in candidates:
        x = max(1, int(x))
        if x not in sizes:
...
def _build_stage_timesteps(
    total_timesteps: int,
    num_stages: int,
    stage_steps_cfg: List[int],
    stage_ratio_cfg: List[float],
) -> List[int]:
    total = max(int(total_timesteps), 1)
    n = max(int(num_stages), 1)

    if stage_steps_cfg:
        steps = [max(int(x), 1) for x in stage_steps_cfg[:n]]
        if len(steps) < n:
            steps.extend([steps[-1]] * (n - len(steps)))
        diff = total - int(sum(steps))
        steps[-1] = max(1, steps[-1] + diff)
        return steps

    if stage_ratio_cfg:
        ratios = [max(float(x), 0.0) for x in stage_ratio_cfg[:n]]
        if len(ratios) < n:
            ratios.extend([ratios[-1] if ratios else 1.0] * (n - len(ratios)))
        s = float(sum(ratios))
        if s <= 0:
            ratios = [1.0] * n
            s = float(n)
        ratios = [r / s for r in ratios]
        steps = [max(1, int(round(total * r))) for r in ratios]
        diff = total - int(sum(steps))
        steps[-1] = max(1, steps[-1] + diff)
        return steps
    ...
    base = total // n
    rem = total - base * n
    steps = [max(1, base) for _ in range(n)]
    steps[-1] += rem
    return steps


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train MAPPO with curriculum learning for UAV-SFC scheduling.")
    parser.add_argument(
        "--config",
        type=str,
        default=os.path.join("MAPPO", "config_small_curriculum.yaml"),
        help="训练配置 YAML 路径。",
    )
...
def main() -> None:
    ...
    dataset_force_regenerate = bool(dataset_cfg.get("force_regenerate", False))

    # 课程学习配置（仅调度训练集数据多样性，不改模型/优化器/环境逻辑）
    curriculum_cfg = yaml_cfg.get("curriculum", {}) if isinstance(yaml_cfg.get("curriculum"), dict) else {}
    curriculum_enabled = bool(curriculum_cfg.get("enabled", True))
    stage_dataset_episodes_cfg = curriculum_cfg.get("stage_dataset_episodes")
    if isinstance(stage_dataset_episodes_cfg, list) and stage_dataset_episodes_cfg:
        stage_dataset_episodes = [max(1, int(x)) for x in stage_dataset_episodes_cfg]
    else:
        stage_dataset_episodes = _build_default_curriculum_sizes(dataset_num_episodes)
    stage_steps_cfg_raw = curriculum_cfg.get("stage_timesteps", [])
    stage_steps_cfg = [int(x) for x in stage_steps_cfg_raw] if isinstance(stage_steps_cfg_raw, list) else []
    stage_ratio_cfg_raw = curriculum_cfg.get("stage_step_ratios", [])
    stage_ratio_cfg = [float(x) for x in stage_ratio_cfg_raw] if isinstance(stage_ratio_cfg_raw, list) else []
    ...
    experiment_name = args.experiment_name or f"mappo_curr_seed{args.seed}_{stamp}"
    ...
    runtime_cfg = {
        "runtime": {
            ...
            "dataset_force_regenerate": dataset_force_regenerate,
            "curriculum_enabled": curriculum_enabled,
            "curriculum_stage_dataset_episodes": stage_dataset_episodes,
            "curriculum_stage_timesteps_cfg": stage_steps_cfg,
            "curriculum_stage_step_ratios_cfg": stage_ratio_cfg,
    ...
    # 课程学习训练集：按阶段准备多个数据集（相同seed体系，不同episode多样性）
    if not curriculum_enabled:
        stage_dataset_episodes = [dataset_num_episodes]

    curriculum_datasets = []
    for stage_i, stage_eps in enumerate(stage_dataset_episodes, start=1):
        ds = create_or_load_dataset(
            base_seed=dataset_base_seed,
            num_episodes=stage_eps,
            data_dir=dataset_data_dir,
            force_regenerate=dataset_force_regenerate,
            num_locations=int(scene_cfg.get("num_locations", env_config.NUM_LOCATIONS)),
            area_size=float(scene_cfg.get("area_size", env_config.AREA_SIZE)),
            num_uavs=int(scene_cfg.get("num_uavs", env_config.NUM_UAVS)),
            num_requests=int(scene_cfg.get("num_requests", env_config.NUM_REQUESTS)),
        ...
        print(
            f"[MAPPO][CUR] stage={stage_i} train dataset ready: episodes={len(ds)} "
            f"seed={dataset_base_seed} dir={dataset_data_dir}",
            flush=True,
        )

    env = MAPPOSFCEnv(config_yaml_path=args.config, episode_data=curriculum_datasets[0][0], seed=args.seed)
    env.set_request_shuffle_on_reset(shuffle_requests_on_reset)

    # 每次 episode reset 前切换到当前阶段数据集中的下一条数据
    _raw_env_reset = env.reset
    dataset_state = {"active": curriculum_datasets[0], "idx": 0}

    def _reset_with_dataset_rotation():
        active_ds = dataset_state["active"]
        ep_data = active_ds[dataset_state["idx"] % len(active_ds)]
    ...
    rollout_steps_for_train = int(args.rollout_steps) if args.rollout_steps is not None else int(trainer_cfg.rollout_steps)
    training_state = {"update_offset": 0}
        ...
        with torch.no_grad():
            for i in range(eval_num_eval_episodes):
                ...
                while not done:
                    ...
                    obs, reward, done, info = eval_env.step(agent_id, action)
    ...
    def _on_update(stats: Dict[str, float]) -> None:
        ...
        stats = dict(stats)
        stats["update"] = float(int(stats.get("update", 0)) + int(training_state["update_offset"]))
        all_logs.append(dict(stats))
    ...
    if train_total_timesteps is None:
        train_total_timesteps = int(trainer_cfg.total_timesteps)
    stage_timesteps = _build_stage_timesteps(
        total_timesteps=int(train_total_timesteps),
        num_stages=len(curriculum_datasets),
        stage_steps_cfg=stage_steps_cfg if curriculum_enabled else [],
        stage_ratio_cfg=stage_ratio_cfg if curriculum_enabled else [],
    )

    for stage_idx, (stage_ds, stage_steps) in enumerate(zip(curriculum_datasets, stage_timesteps), start=1):
        dataset_state["active"] = stage_ds
        dataset_state["idx"] = 0
        trainer.current_obs = env.reset()

        stage_updates = max(int(stage_steps) // max(rollout_steps_for_train, 1), 1)
        print(
            f"[MAPPO][CUR] stage={stage_idx}/{len(curriculum_datasets)} "
            f"dataset_episodes={len(stage_ds)} stage_steps={int(stage_steps)} "
            f"stage_updates={stage_updates}",
            flush=True,
        )
        trainer.train(
            total_timesteps=int(stage_steps),
            rollout_steps=rollout_steps_for_train,
            log_interval_updates=args.log_interval,
            update_callback=_on_update,
            step_callback=_on_step,
            episode_callback=_on_episode,
        )
        training_state["update_offset"] += stage_updates
    ...
    logger.log_checkpoint(
        ...
        metrics={
            "avg_success_rate_10ep": float(all_logs[-1]["avg_success_rate_10ep"]) if all_logs else 0.0,
            "avg_return_10ep": float(all_logs[-1]["avg_return_10ep"]) if all_logs else 0.0,
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        yaml.safe_dump(all_logs, f, allow_unicode=True, sort_keys=False)
    if record_step_metrics:
        step_file.close()
    update_file.close()
    episode_file.close()
